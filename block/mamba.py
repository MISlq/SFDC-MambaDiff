...
############################################
class Z_CrossScan(torch.autograd.Function):
    @staticmethod
...
class Z_CrossMerge(torch.autograd.Function):
    @staticmethod
...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, scan_type, inference_params=None):
        ...
        if scan_type == 's':
            xz_list = CrossScan.apply(xz, self.token_list, self.token_list_reversal, self.origina_list,
                                      self.origina_list_reversal)
            ...
            xz_ = Z_CrossScan.apply(xz, self.token_list, self.origina_list)
            ...
            out = (out1 + out2) / 2
            out = Z_CrossMerge.apply(out, self.token_list, self.origina_list)
